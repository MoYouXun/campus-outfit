...
public class RankingController {
    ...
    @GetMapping("/hot")
    public Result<List<Outfit>> getHotRanking(@RequestParam(defaultValue = "10") int limit) {
        return Result.success(rankingService.getHotRanking(limit));
    }

    @GetMapping("/style")
    public Result<List<Outfit>> getStyleRanking(@RequestParam String style, @RequestParam(defaultValue = "10") int limit) {
        return Result.success(rankingService.getStyleRanking(style, limit));
    }

    @GetMapping("/school")
    public Result<List<Outfit>> getSchoolRanking(@RequestParam String school, @RequestParam(defaultValue = "10") int limit) {
